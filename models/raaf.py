...
# ==============================================================================
    def forward(self, inputs):
        ...
        """
        # Extract modality inputs (RGB and Depth streams)
        f_rgb, f_d = inputs[0], inputs[1]
        
        # --- Stage 1: Modality-Specific Attention Refinement ---
        # Extract geometric boundary attention from the depth stream
        attn_d = self.geom_attn_d(f_d)   
        f_d = f_d * attn_d        
        
        # Extract standard spatial attention from the RGB stream
        ...
        # --- Stage 2: Depth Reliability Assessment ---
        # Assess spatial credibility of the depth map (C_d in the paper)
        conf_d = self.reliability_branch(f_d)  

        # --- Stage 3: Reliability-Guided Asymmetric Interaction ---
        # Calculate cross-modal differential potential
        f_diff = f_d - f_rgb
        
        # If depth is reliable (conf_d -> 1): Inject geometric priors to enhance RGB
        f_rgb_enh = f_rgb + conf_d * self.geom_injector(f_diff)
        
        # If depth is unreliable (conf_d -> 0): Utilize RGB semantics to compensate Depth voids
        f_d_enh = f_d + (1.0 - conf_d) * self.semantic_compensator(-f_diff)

        # --- Stage 4: Competitive Softmax Weight Generation ---
        # Obtain feature response logits
        logits_rgb = self.weight_gen_rgb(f_rgb_enh)
        logits_d = self.weight_gen_d(f_d_enh)   
        
        # Compete and normalize across the modality dimension (dim=1)
        ...
        # --- Stage 5: Dynamic Weighted Fusion ---
